      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03]">
        {faqs.length === 0 ? (
          ...
          <div className="divide-y divide-white/10">
            {faqs.map((faq) => (
              <div key={faq.id} className="grid gap-3 px-5 py-4 hover:bg-white/[0.03] lg:grid-cols-[1fr_auto_auto_auto] lg:items-center">
                <div>
                  <Link href={`/admin/content/faqs/${faq.id}`} className="font-semibold text-white hover:text-teal">{faq.question}</Link>
                ...
                <ContentLifecycleForm entityType="FAQ" id={faq.id} archived={faq.status === ContentStatus.ARCHIVED} />
              </div>
            ))}
          </div>
        )}
