        ...
        {testimonials.length === 0 ? (
          ...
          <div className="divide-y divide-white/10">
            {testimonials.map((item) => (
              <div key={item.id} className="grid gap-3 px-5 py-4 hover:bg-white/[0.03] lg:grid-cols-[1fr_auto_auto_auto] lg:items-center">
                <div>
                  <Link href={`/admin/content/testimonials/${item.id}`} className="font-semibold text-white hover:text-teal">{item.person}</Link>
                ...
                <ContentLifecycleForm entityType="Testimonial" id={item.id} archived={item.status === ContentStatus.ARCHIVED} />
              </div>
            ))}
          </div>
        )}
